...
def main(environment, ll_username, ll_password, aws_profile_name, target_account_ids):
    ...
    print(color("Fetching target accounts", "blue"))

    # Getting only the account IDs of the active AWS accounts
    list_accounts = get_all_accounts(org_client)
    target_accounts = [(a["Id"], a["Name"]) for a in list_accounts if a["Status"] == "ACTIVE" and a["Id"] in target_account_ids]
    print(color(f"Found {len(target_accounts)} target accounts", "blue"))

    # Setting the dict for successfully integrated accounts
    accounts_integrated = {}

    for sub_account in target_accounts:
        ...
        try:
            ...
            # ... (the rest of the code from the original script that was inside the loop)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description='This script will integrate Lightlytics environment with the specified accounts in the organization.')
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default="staging")
    parser.add_argument(
        "--target_account_ids", help="The AWS account IDs of the target accounts, separated by commas", required=True)
    args = parser.parse_args()
    target_account_ids = args.target_account_ids.split(',')
    main(args.environment_sub_domain, args.environment_user_name, args.environment_password, args.aws_profile_name, target_account_ids)
